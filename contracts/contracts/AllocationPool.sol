...
contract AllocationPool is Initializable, OwnableUpgradeable {
    ...
    function __AllocationPool_init(
        ...
        require(
            ...
            "Alloc: invalid reward token address"
        );
    ...
    modifier allocValidatePoolById(uint256 _pid) {
        require(_pid < allocPoolInfo.length, "Alloc: pool are not exist");
        _;
    }
    ...
    function allocAddPool(
        ...
        require(!allocIsAdded[_lpToken], "Alloc: pool already is added");
        require(
            _delayDuration <= ALLOC_MAXIMUM_DELAY_DURATION,
            "Alloc: delay duration is too long"
        );
     ...
     */
    function allocSetRewardDistributor(
        address _allocRewardDistributor
    ) external onlyOwner {
        require(
            _allocRewardDistributor != address(0),
            "Alloc: invalid reward distributor"
        );
    ...
    function allocSetEndBlock(uint64 _endBlockNumber) external onlyOwner {
        require(
            ...
            "Alloc: invalid end block number"
        );
     ...
     * @param _rewardPerBlock the number of reward tokens that got unlocked each block
     */
    function allocSetRewardPerBlock(
        uint128 _rewardPerBlock
    ) external onlyOwner {
        allocMassUpdatePools();
        allocRewardPerBlock = _rewardPerBlock;
     ...
     */
    function allocSetDisabledPool(
        uint128 _pid,
        bool _shouldDisabled
    ) external onlyOwner allocValidatePoolById(_pid) {
     ...
     */
    function allocPendingReward(
        uint256 _pid,
        address _user
    ) public view allocValidatePoolById(_pid) returns (uint128) {
     ...
     * @param _pid id of the pool
     */
    function allocClaimPendingWithdraw(
        uint256 _pid
    ) external allocValidatePoolById(_pid) {
        AllocPoolInfo storage pool = allocPoolInfo[_pid];
        ...
        require(amount > 0, "Alloc: nothing is currently pending");
        require(
            pendingWithdraw.applicableAt <= block.timestamp,
            "Alloc: not released yet"
        );
     ...
     */
    function allocSetAllowEmergencyWithdraw(
        bool _shouldAllow
    ) external onlyOwner {
     ...
     * @param _pid id of the pool
     */
    function allocEmergencyWithdraw(
        uint256 _pid
    ) external allocValidatePoolById(_pid) {
        require(
            allocAllowEmergencyWithdraw,
            "Alloc: emergency withdrawal is not allowed yet"
        );
        AllocPoolInfo storage pool = allocPoolInfo[_pid];
        AllocUserInfo storage user = allocUserInfo[_pid][msg.sender];
     ...
     * @param _pid id of the pool
     */
    function allocClaimReward(
        uint256 _pid
    ) public allocValidatePoolById(_pid) returns (uint128) {
        allocUpdatePool(_pid);
        AllocPoolInfo storage pool = allocPoolInfo[_pid];
        AllocUserInfo storage user = allocUserInfo[_pid][msg.sender];
    ...
    function _allocWithdraw(
        ...
        AllocPoolInfo storage pool = allocPoolInfo[_pid];
        AllocUserInfo storage user = allocUserInfo[_pid][msg.sender];
        require(user.amount >= _amount, "Alloc: invalid amount");
    ...
    function allocSafeRewardTransfer(address _to, uint128 _amount) internal {
        ...
        require(_amount <= bal, "Alloc: not enough reward token");
